			     ...
			     enum typec_cc_polarity polarity)
{
	...
	dev_info(chip->dev, "tcpm_set_polarity 0x%x\n",polarity);
...
static int tcpm_set_vconn(struct fusb30x_chip *chip, int enable)
{
	...
	dev_info(chip->dev, "tcpm_set_vconn 0x%x\n",enable);
...
static void process_vdm_msg(struct fusb30x_chip *chip)
{
	...
	dev_info(chip->dev, "VDM CMD:%x\n",GET_VDMHEAD_CMD_TYPE(vdm_header));
	...
	case VDM_TYPE_ACK:
		...
		case VDM_DP_STATUS_UPDATE:
			...
			dev_dbg(chip->dev, "DisplayPort Status: 0x%08x\n",
				chip->rec_load[1]);
			dev_info(chip->dev, "DisplayPort Status: 0x%08x\n",
				chip->rec_load[1]);	
			chip->val_tmp = 1;
			break;
		case VDM_DP_CONFIG:
...
static void fusb_state_unattached(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev,"fusb_state_unattached 0x%x 0x%x\n",evt,chip->cc_state);
...
static void fusb_state_attach_wait_sink(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev,"fusb_state_attach_wait_sink 0x%x\n",evt);
...
{
	dev_info(chip->dev,"fusb_state_attached_sink 0x%x\n",evt);
...
static void fusb_state_try_attach(struct fusb30x_chip *chip, u32 evt,
				  enum role_mode mode)
{
	dev_info(chip->dev,"fusb_state_try_attach 0x%x\n",evt);
					     ...
					     u32 evt)
{
	...
	dev_info(chip->dev,"fusb_state_src_transition_supply 0x%x\n",chip->sub_state);
...
static void fusb_state_src_cap_response(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev,"fusb_state_src_cap_response 0x%x\n",evt);
...
{
	dev_info(chip->dev, "fusb_state_src_ready evt:0x%x\n",evt);
...
{
	dev_info(chip->dev,"fusb_state_prs_reject 0x%x\n",evt);
...
{
	dev_info(chip->dev, "fusb_state_prs_accept evt:0x%x\n",evt);
...
{
	dev_info(chip->dev, "fusb_state_vcs_ufp_accept evt:0x%x\n",evt);
...
static void fusb_state_vcs_set_vconn(struct fusb30x_chip *chip,
				     u32 evt, bool on)
{
	dev_info(chip->dev, "fusb_state_vcs_set_vconn evt:0x%x 0x%x\n",evt,on);
...
{
	dev_info(chip->dev, "fusb_state_vcs_send_ps_rdy evt:0x%x\n",evt);
...
static void fusb_state_src_prs_source_off(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev, "fusb_state_src_prs_source_off evt:0x%x\n",evt);
...
static void fusb_state_drs_send_accept(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev,"fusb_state_drs_send_accept 0x%x\n",evt);
...
{
	dev_info(chip->dev, "fusb_state_drs_role_change evt:0x%x\n",evt);
...
static void fusb_state_src_softreset(struct fusb30x_chip *chip)
{
	...
	dev_info(chip->dev, "fusb_state_src_softreset chip->sub_state:0x%x\n",chip->sub_state);
...
{
	dev_info(chip->dev, "fusb_state_snk_startup evt:0x%x\n",evt);
...
{
	dev_info(chip->dev, "fusb_state_snk_discovery evt:0x%x\n",evt);
...
{
	dev_info(chip->dev, "fusb_state_snk_wait_caps evt:0x%x\n",evt);
...
static void fusb_state_snk_evaluate_caps(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev, "fusb_state_snk_evaluate_caps evt:0x%x\n",evt);
...
static void fusb_state_snk_select_cap(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev, "fusb_state_snk_select_cap evt:0x%x\n",evt);
...
{
	dev_info(chip->dev,"fusb_state_snk_transition_sink evt:0x%x\n",evt);
...
static void fusb_state_snk_transition_default(struct fusb30x_chip *chip,
					      u32 evt)
{
	dev_info(chip->dev,"fusb_state_snk_transition_default evt:0x%x\n",evt);
...
static void fusb_state_snk_ready(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev, "fusb_state_snk_ready evt:0x%x\n",evt);
...
static void fusb_state_send_swap(struct fusb30x_chip *chip, u32 evt, int cmd)
{
	...
	dev_info(chip->dev,"fusb_state_send_swap 0x%x 0x%x\n",evt,cmd);
...
static void fusb_state_snk_prs_source_on(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev,"fusb_state_snk_prs_source_on 0x%x\n",evt);
...
static void fusb_state_snk_softreset(struct fusb30x_chip *chip)
{
	...
	dev_info(chip->dev,"fusb_state_snk_softreset 0x%x\n",chip->sub_state);
...
static void fusb_state_snk_send_softreset(struct fusb30x_chip *chip, u32 evt)
{
	...
	dev_info(chip->dev,"fusb_state_snk_send_softreset evt:0x%x\n",evt);
